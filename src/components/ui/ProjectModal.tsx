    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 overflow-y-auto">
        ...
        <motion.div
          ...
          className="relative w-full max-w-4xl bg-[#121316] border-2 border-cyan-500/40 rounded-[36px] p-6 sm:p-10 text-[#D7E2EA] shadow-2xl z-10 my-8 max-h-[90vh] overflow-y-auto"
        >
          {/* Close Button */}
          <button
            onClick={onClose}
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 mb-6 border-b border-white/10 pb-6">
            <div>
              <div className="flex items-center gap-3 mb-2">
                <span className="px-3 py-1 rounded-full bg-cyan-950/80 border border-cyan-500/40 text-xs font-bold uppercase tracking-wider text-cyan-300 font-mono">
                  {project.status}
                </span>
              ...
              <p className="text-sm sm:text-base text-cyan-300/90 font-medium mt-1 font-mono">
                {project.subtitle}
              </p>
            </div>
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              {project.github && (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full border border-white/20 hover:bg-white/10 text-white font-medium text-xs font-mono uppercase tracking-wider transition-colors"
                >
          ...
          <div className="mb-8">
            <h3 className="text-xs uppercase tracking-widest text-white/40 font-mono font-semibold mb-2">
              System Overview &amp; Backend Role
            </h3>
          ...
          {project.architecture && project.architecture.length > 0 && (
            <div className="mb-8 p-6 rounded-3xl bg-black/40 border border-cyan-500/30">
              <h3 className="text-xs uppercase tracking-widest text-cyan-400 font-mono font-semibold mb-4 flex items-center gap-2">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
                {project.architecture.map((arch, i) => (
                  ...
                  >
                    <div>
                      <span className="text-2xl font-black text-cyan-400 font-kanit">
                        {arch.step}
                      </span>
                      <h4 className="text-sm font-bold text-white mt-1 font-kanit">
                        {arch.label}
                      </h4>
                      <p className="text-xs text-[#D7E2EA]/60 mt-1 leading-snug font-mono">
                        {arch.desc}
                      </p>
                    </div>
                    {i < project.architecture.length - 1 && (
                      <ArrowRight className="hidden lg:block absolute -right-3 top-1/2 -translate-y-1/2 w-5 h-5 text-cyan-400 z-10 bg-[#121316] rounded-full p-0.5 border border-white/10" />
                    )}
                  </div>
                ))}
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            ...
            <div className="p-6 rounded-3xl bg-[#17181D] border border-white/10">
              ...
              <ul className="space-y-2.5 text-sm text-[#D7E2EA]/80 font-light">
              ...
              <div className="space-y-4">
                {project.decisions.map((dec) => (
                  <div key={dec.title}>
                    ...
                    <p className="text-xs sm:text-sm text-[#D7E2EA]/70 mt-1 leading-snug font-light">
                      {dec.reason}
                    </p>
                  </div>
                ))}
          ...
          <div className="p-6 rounded-3xl bg-[#141519] border border-white/10 mb-8 space-y-4">
            <div>
              <h4 className="text-xs uppercase tracking-widest text-red-400 font-mono font-bold mb-1 flex items-center gap-1.5">
                <AlertCircle className="w-3.5 h-3.5" /> Technical Challenge Resolved
              </h4>
              <p className="text-sm text-[#D7E2EA]/80 font-light">{project.challenges}</p>
            </div>
            <div>
              <h4 className="text-xs uppercase tracking-widest text-cyan-400 font-mono font-bold mb-1 flex items-center gap-1.5">
                <Lightbulb className="w-3.5 h-3.5" /> Key Engineering Takeaway
              </h4>
              <p className="text-sm text-[#D7E2EA]/80 font-light">{project.learnings}</p>
            </div>
          </div>
          ...
          <div className="pt-6 border-t border-white/10 flex flex-wrap items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3">
              {project.metrics.map((m) => (
                <span key={m} className="px-3 py-1 rounded-full bg-cyan-950/80 border border-cyan-500/40 text-cyan-300 text-xs font-mono font-semibold">
                  ✓ {m}
                </span>
              ))}
            </div>
